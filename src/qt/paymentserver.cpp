...
const int BITBREADCRUMB_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BITBREADCRUMB_IPC_PREFIX("duckcoin:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/duckcoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/duckcoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/duckcoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITBREADCRUMB_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BITBREADCRUMB_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(BITBREADCRUMB_IPC_CONNECT_TIMEOUT);
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start duckcoin: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITBREADCRUMB_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                emit message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Duckcoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
