...
CAmount AmountFromValue(const Value& value)
{
    ...
    CAmount nAmount = roundint64(dAmount * BREADCRUMB);
...
{
    return (double)amount / (double)BREADCRUMB;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Duckcoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Duckcoin server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use duckcoind, or the -server option to duckcoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> duckcoin-cli " + methodname + " " + args + "\n";
}
