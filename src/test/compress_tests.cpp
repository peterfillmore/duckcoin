...
BOOST_AUTO_TEST_CASE(compress_amounts)
{
    ...
    BOOST_CHECK(TestPair(         BREADCRUMB,       0x9));
    BOOST_CHECK(TestPair(      50*BREADCRUMB,      0x32));
    BOOST_CHECK(TestPair(21000000*BREADCRUMB, 0x1406f40));
    ...
    for (uint64_t i = 1; i <= NUM_MULTIPLES_1BTC; i++)
        BOOST_CHECK(TestEncode(i * BREADCRUMB));

    for (uint64_t i = 1; i <= NUM_MULTIPLES_50BTC; i++)
        BOOST_CHECK(TestEncode(i * 50 * BREADCRUMB));
