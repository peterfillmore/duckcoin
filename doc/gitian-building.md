...
*Setup instructions for a Gitian build of Duckcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Duckcoin
...
- [Building Duckcoin](#building-duckcoin)
...
Clone the git repositories for duckcoin and Gitian and then checkout the duckcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/duckcoin-project/duckcoin.git
cd duckcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the duckcoin repository
...
To build Duckcoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the duckcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/duckcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/duckcoin-project/duckcoin
...
For example:
```bash
URL=https://github.com/user/duckcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit duckcoin=${COMMIT} --url duckcoin=${URL} ../duckcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit duckcoin=${COMMIT} --url duckcoin=${URL} ../duckcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit duckcoin=${COMMIT} --url duckcoin=${URL} ../duckcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/duckcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/duckcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/duckcoin-build.assert
```
...
[duckcoin/gitian.sigs](https://github.com/duckcoin-project/gitian.sigs.duk/) repository, or if that's not possible create a pull
request. 
